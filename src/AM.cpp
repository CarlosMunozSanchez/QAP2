...
AM::AM(int tipo, int hibridacion, int n_genes, const vector<vector<int>> & flujos, 
...
pair<vector<int>, vector<int>> AM::cruce(const vector<int> & c1, 
...
void AM::mutacion(vector<int> & cromosoma){
...
void AM::simularEvolucion(const vector<vector<int> >& flujos, 
    ...
    while(evaluaciones < MAX_EVAL){
        ...
        //5. APLICACIÓN DE BL DE BAJA INTENSIDAD
        //cada 10 iteraciones, ejecuto BL
        if(T % frecBL){
            //Según el tipo de hibridación
            switch(hibridacion){
                case 0:{ //0 -> (10, 1), BLs sobre toda la población
                    for(int j = 0; j < POBLACION_SIZE; j++){
                        //aplico búsqueda local
                        QAPBL bl(poblacion[j], flujos, distancias, 42);
                ...
                case 1:{ //1 -> (10,0.1)
                    //Obtengo 10 indices aleatorios
                    vector<int> indices;
                    int N = POBLACION_SIZE * 0.1;
                    for(int i = 0; i < N; i++){
                        indices.push_back(Random::get(0, (int)poblacion[0].size()));
                    }
                ...
                case 2:{ //2 -> (10, 0.1mej)
                    int N = POBLACION_SIZE * 0.1;
                    //sé a priori que el 10% de 50 es 5, así que puedo usar la lista
                    //de mejoresPadres para aplicar BL sobre ellos
                    multimap<int, int>::iterator it = fitnessMejoresPadres.begin();
                    
                    for(int i = 0; i < N; i++){
                        QAPBL bl(poblacion[it->second], flujos, distancias, 42);
        ...
        for(int i = 0; i < POBLACION_SIZE; i++){
            ...
            //para los siguientes, tengo que comprobar si son mejores que los que ya hay
                ...
                //otengo el padre con el peor fitness, que está al final del multimap
                multimap<int, int>::iterator it = fitnessMejoresPadres.end();
                it--;
